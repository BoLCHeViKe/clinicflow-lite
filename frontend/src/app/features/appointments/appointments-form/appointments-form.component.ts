...
export class AppointmentsFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.isEditing) {
      this.appointmentsService.getById(this.itemId!).subscribe(res => {
        const appt = res.data;
        // Server sends '2026-04-28 09:00:00' — replace space with T and trim to 'YYYY-MM-DDTHH:mm'
        // Do NOT use toISOString() here: it converts to UTC and shifts the hour in non-UTC timezones
        const localDate = appt.appointmentDate.replace(' ', 'T').slice(0, 16);
  ...
  onSubmit(): void {
    ...
    const base = {
      ...
      // datetime-local gives 'YYYY-MM-DDTHH:mm' (local time) — keep it local, do NOT use toISOString()
      appointmentDate: raw.appointmentDate
        ? raw.appointmentDate.replace('T', ' ') + ':00'
        : '',
